...
TM_Frame_t* C_TelemetryManager::updateTM()
{
    ...
    frame->timeStamp = mBCIPackagePtr->currentTime().elapsed();
    frame->brsFrame  = mBRS_IOPtr->GetLatestBRSFrame();
    frame->eegFrame  = mEEG_IOPtr->getData().GetFrame();
...
{
    debugLog->BCI_Log() << "Recording Telemetry to File: " << filename.toStdString() << endl;
